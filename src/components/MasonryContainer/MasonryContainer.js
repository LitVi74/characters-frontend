...
export default function MasonryContainer({cbShow, cbClose, cbPlus, spells, charList}) {
  ...
  return (
    <section className="masonry__container" ref={ref}>
      {spells.map((spell) =>
      <MasonryCard key={spell._id} cbShow={cbShow} spell={spell} charList={charList} cardWidth={cardWidth} cbClose={cbClose} cbPlus={cbPlus} />
      )}
    </section>
  );
}